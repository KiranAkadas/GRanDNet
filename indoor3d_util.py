...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    data_label = np.concatenate((points_list,), 1)
...
def collect_point_label_nw_txt(dir1, out_filename, file_format='txt'):
#    for f in glob.glob(os.path.join("train/", '*.txt')):
#        cls = os.path.basename(f).split(' ')[-1]
#    if cls not in nw_classes: # note: in some room there is 'staris' class..
#        cls = 'clutter'
    f=open(dir1)
    flag=0
    points_list=[]
    for i in f:
         if flag==1:
            a1,a2,a3,l=i.split()
            x=np.ones((1,1))*[float(a1),float(a2),float(a3)]
            #print(float(a1))
            #print(l)
            r,g,b=nw_class2color[l]
            y=np.ones((1,1))*[r,g,b,int(l)]
            points_list.append(np.concatenate([x,y], 1))
         flag=1
...
def collect_point_label_nw(dir1, out_filename, file_format='txt'):
#    for f in glob.glob(os.path.join("train/", '*.txt')):
#        cls = os.path.basename(f).split(' ')[-1]
#    if cls not in nw_classes: # note: in some room there is 'staris' class..
#        cls = 'clutter'
