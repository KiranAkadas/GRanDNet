...
class Semantic3D:
    def __init__(self):
        ...
        self.path = './data_train/'
        self.testpath = './data_test/'
        ...
        self.original_test_folder = join(self.testpath, 'original_data')
        self.full_pc_test_folder = join(self.testpath, 'original_ply')
        self.sub_pc_folder = join(self.path, 'input_{:.3f}'.format(cfg.sub_grid_size))

        # Following KPConv to do the train-validation split
        self.all_splits = [0, 1, 4, 5, 3, 1, 3, 0, 1, 2, 3, 4, 1, 0, 5,0, 1, 4, 5, 3, 4, 1, 0, 1, 2, 3, 4, 2, 0, 5,0, 1, 4, 5, 3, 1, 1, 0, 1, 2, 3, 4, 2, 0, 5,0, 1, 4, 1, 3, 4, 3, 0, 1, 2, 3, 4, 1, 0, 5]
        self.val_split = 1
        ...
        self.val_files = []
        self.test_files = []
        cloud_names = [file_name[:-4] for file_name in os.listdir(self.original_test_folder) if file_name[-4:] == '.txt']
        for pc_name in cloud_names:
            print(pc_name)
            if True:
                self.test_files.append(join(self.full_pc_test_folder, pc_name + '.ply'))
        print("TRAIN:",self.train_files)
    ...
    def get_batch_gen(self, split):
       ...
       # if split != 'test':
        ...
        #print(queried_pc_xyz,queried_pc_labels,query_idx.astype(np.int32),np.array([cloud_idx]))
...
if __name__ == '__main__':
    ...
    if Mode == 'train':
        ...
        if FLAGS.model_path is not 'None':
            chosen_folder = FLAGS.model_path
